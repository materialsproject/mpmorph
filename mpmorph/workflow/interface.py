...
def get_interface_wf(interfaces, match, slab_substrates=None, slab_films=None,
    ...
    if slab_substrates is not None:
        for i, slab_substrate in enumerate(slab_substrates):
            ...
            fw_list.append(sub_slab_static_fw)
        ...
        for i, slab_film in enumerate(slab_films):
            if md_prerelax:
                ...
                fw = MDFW(structure=slab_film,
                          name=f'{film_label}_{i}-{h_label}_film_slab_md_prerelax_0-{str(tag_id)}',
...
def get_amorphous_interface_wf(interfaces, interface_labels, slab_films, md_prerelax=True,
                               substrate_layers=None, md_temp=500, h_label='bare_surface'):
    """
    This workflow currently does not allow for calculation of interfacial energy.
    ...
    sub_miller = "".join([str(i) for i in interfaces[0].sub_plane])
    sub_comp = interfaces[0].substrate.composition.reduced_formula
    film_comp = interfaces[0].film.composition.reduced_formula

    sub_label = f'{substrate_layers}_{sub_comp}({sub_miller})'
    film_label = f'{len(interfaces[0].get_film_indices())}_atoms_amorph-{film_comp}'
    label = f'{film_label}/{sub_label}'
    tag_id = uuid.uuid4()
